...
class Terraform:
    ...
    local_terraform_resources_path = os.getenv(
        "LOCAL_TERRAFORM_RESOURCES_PATH", "./terraform_resources/"
    )
...
class UTM:
    ...
    utm_services_api = os.getenv(
        "UTM_SERVICES_API", "api/v2/cmdb/firewall.service/custom"
    )
...
class LDAP:
    ...
    groups_dn = os.getenv(
        "LDAP_GROUPS_DN", "OU=UTM ACCESS,OU=Groups,OU=ASAX Objects,DC=asax,DC=local"
    )
...
class Celery:
    ...
    celery_result_backend = os.getenv(
        "CELERY_RESULT_BACKEND", f"redis://{Redis.host}:{Redis.port}/0"
    )
