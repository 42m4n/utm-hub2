...
def create_requests_obj(udf_fields):
    ...
    if new_object["action"] == "Grant":
        new_object["action"] = "accept"
    if new_object["action"] == "Deny":
        new_object["action"] = "deny"
...
def create_requests_list(udf_fields):
    """
    This code block get raw dictionary of ticket fields
    and group them by request rows and make list of access requests
    Args:
        udf_fields: dictionary of all access requests fields in a ticket

    Returns:
        requests_list: list of objects that each object has
        source_name,destination_name,port,action and schedule
    """
    rows = [
        [
            "udf_sline_3324",
...
def fill_trf_fields(policy_name, data, file_path, template_name: str, utm_name):
    ...
    try:
        ...
        file_loader = FileSystemLoader(
            os.path.join(settings.BASE_DIR, "apps", "paasapp", "templates")
        )
        ...
        if dest_ipaddr_json == []:
            raise ValueError(
                "destination address is invalid or it's not in asset management yet."
            )
        ...
        if data.get("access_type") == "Server to Server":
            ...
            if src_ipaddr_json == []:
                raise ValueError(
                    "source address is invalid or it's not in asset management yet."
                )
...
def convert_policies_to_terraform_file(policies: list, file_path, data, ticket_number):
    ...
    for policy in policies:
        ...
        # Regex pattern to find existing FortiGate policy resources in Terraform file
