...
def send_response_to_manage_engine(request_id, source, destination, service, response_code):
    """
    This function update ticket in Manage Engine by add a note to ticket contains the result of utm
    and if the response code is ok change the ticket status to close.
    """

    ticket_url = f"{conf.ManageEngine.manage_engine_address}/api/v3/requests/{request_id}"
    ...
    status_input_data = {
        "request": {
            "status": {
                "id": conf.ManageEngine.manage_engine_done_status
            }
        }}
    comment_data = {'input_data': str(comment_input_data)}
    status_data = {'input_data': str(status_input_data)}
    try:
        logger.info(f'Sending data : {comment_input_data} to ManageEngine to add comment')
        response = requests.post(comment_url, headers=headers, data=comment_data, verify=False)
        ...
        if response_code == "1":
            logger.info(f'Sending data : {status_data} to ManageEngine to change ticket status')
            response = requests.put(ticket_url, headers=headers, data=status_data, verify=False)
