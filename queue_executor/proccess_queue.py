...
from conf import UTM, BaseSetting
from conf import Redis as RedisConf
from conf import Terraform as TerraformConf
...
def format_terraform_result(terraform_result):
    try:
        ...
        if ret_code == 0:
            output = re.findall(
                r"Plan: (\d+) to add, (\d+) to change, (\d+) to destroy", out
            )
            if output:
                result_dict["add"], result_dict["change"], result_dict["destroy"] = map(
                    int, output[0]
                )
...
def apply_terraform(queue_object, var_utm_token):
    files_directory = (
        TerraformConf.local_terraform_resources_path
        if BaseSetting.debug
        else TerraformConf.terraform_resources_path
    )
    ...
    variables = {"utm_token": var_utm_token}
        ...
        try:
            ...
            if queue_len > 0:
                ...
                try:
                    ...
                    result, success = apply_terraform(
                        queue_object=data, var_utm_token=utm_token
                    )
                    if success:
                        send_response_to_manage_engine(
                            request_id=data.get("ticket_number"),
                            source=data.get("resource").get("group")
                            or data.get("resource").get("user")
                            or data.get("resource").get("source_name"),
                    ...
                    logger.info(
                        f"UTM terraform result for {data.get('ticket_number')}: {result}"
                    )
