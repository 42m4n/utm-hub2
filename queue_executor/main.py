...
from conf import BaseSetting, Terraform
...
def process_quese():
    ...
    while True:
        queue_file = (
            Terraform.local_terraform_queue_path
            if BaseSetting.debug
            else Terraform.terraform_queue_path
        )
        logs_file = (
            Terraform.local_terraform_log_path
            if BaseSetting.debug
            else Terraform.terraform_log_path
        )
        files_directory = (
        ...
        with open(f"{queue_file}", encoding="UTF-8") as whole_queue:
            ...
            for line in whole_queue:
                ...
                try:
                    ...
                    terraform_command = (
                        f"cd {file_location} && sudo terraform apply -auto-approve"
                    )
                    terraform_command_state = subprocess.check_output(
                        terraform_command, text=True, shell=True
                    )
                    ...
                    with open(f"{logs_file}", "a") as log_file:
                        ...
                        if "Apply complete!" in terraform_command_state:
                            ...
                            send_response_to_manage_engine(
                                ...
                                source=json_line["resource"]["group"]
                                or json_line["resource"]["user"]
                                or json_line["resource"]["source_name"],
